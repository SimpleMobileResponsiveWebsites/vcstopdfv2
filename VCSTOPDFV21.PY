...
with st.sidebar:
    ...
    new_version_number = st.text_input("Version Number (e.g., 1.0):", value="1.0")

    if st.button("Add New File"):
        if new_file_name and new_version_number:
            if new_file_name not in st.session_state.codebases:
                # Create new version object with user-provided version number
                new_version = Version(name=new_file_name, version_number=new_version_number)

                # Add to codebases
                st.session_state.codebases[new_file_name] = new_version

                st.success(f"Added file: {new_file_name} (Version {new_version_number})")
            else:
                st.error("File already exists!")
        else:
            st.error("Please enter both a file name and a version number!")

# Main content area
if st.session_state.codebases:
    # Only show the file selector when files exist
    selected_file = st.selectbox(
        "Select File to Work With",
        [""] + list(st.session_state.codebases.keys()),  # The default option is a blank entry
        index=0  # Default to the blank entry
    )

    if selected_file:  # Only show content if a file is selected
